...
def initialize(
    ...
    self.packed_precisions = packed_precisions  # <------- Precisions of self.coins
...
def _transfer_in(
    ...
    if use_eth and _coin == WETH20:
        ...
        if callback_sig == empty(bytes32):
            ...
            assert ERC20(_coin).balanceOf(self) - b == dx  # dev: callback didn't give us self.coins
            #                                          ^------ note: dx cannot
...
def add_liquidity(
    ...
    assert amounts[0] + amounts[1] + amounts[2] > 0  # dev: no self.coins to add
    ...
    if self.future_A_gamma_time > block.timestamp:  # <--- A_gamma is ramping.
        ...
        old_D = self.MATH.newton_D(A_gamma[0], A_gamma[1], xp_old, 0)

    else:

        old_D = self.D

    D: uint256 = self.MATH.newton_D(A_gamma[0], A_gamma[1], xp, 0)
...
def _exchange(
    ...
    assert dx > 0  # dev: do not exchange 0 self.coins
    ...
    if t > block.timestamp:
        ...
        xp[i] = x0                                                         # |
        self.D = self.MATH.newton_D(A_gamma[0], A_gamma[1], xp, 0)              # |
        xp[i] = x1  # <-------------------------------------- ... and restore.
    ...
    y_out: uint256[2] = self.MATH.get_y(A_gamma[0], A_gamma[1], xp, D, j)
    dy = xp[j] - y_out[0]
    xp[j] -= dy
    dy -= 1
...
def tweak_price(
    ...
    if last_prices_timestamp < block.timestamp:
        ...
        alpha: uint256 = self.MATH.wad_exp(
            -convert(
                unsafe_div(
    ...
    if new_D == 0:  #  <--------------------------- _exchange sets new_D to 0.
        D_unadjusted = self.MATH.newton_D(A_gamma[0], A_gamma[1], _xp, K0_prev)

    # ----------------------- Calculate last_prices --------------------------

    last_prices = self.MATH.get_p(_xp, D_unadjusted, A_gamma)
    for k in range(N_COINS - 1):
    ...
    if old_virtual_price > 0:
        ...
        xcp: uint256 = self.MATH.geometric_mean(xp)
    ...
    if virtual_price * 2 - 10**18 > xcp_profit + 2 * rebalancing_params[0]:
        ...
        if norm > adjustment_step:  # <---------- We only adjust prices if the
            ...
            old_virtual_price = unsafe_div(
                10**18 * self.MATH.geometric_mean(xp), total_supply
...
def _claim_admin_fees():
    ...
    D: uint256 = self.MATH.newton_D(A_gamma[0], A_gamma[1], self.xp(), 0)
    self.D = D
...
def get_xcp(D: uint256) -> uint256:
    ...
    return self.MATH.geometric_mean(x)
...
def _calc_withdraw_one_coin(
    ...
    if update_D:  # <-------------- D is updated if pool is undergoing a ramp.
        D0 = self.MATH.newton_D(A_gamma[0], A_gamma[1], xp, 0)
    else:
        D0 = self.D
    ...
    y: uint256 = self.MATH.get_y(A_gamma[0], A_gamma[1], xp, D, i)[0]
...
def calc_token_amount(amounts: uint256[N_COINS], deposit: bool) -> uint256:
    ...
    @notice Calculate LP tokens minted or to be burned for depositing or
            removing `amounts` of self.coins
    @dev Includes fee.
...
def get_dy(i: uint256, j: uint256, dx: uint256) -> uint256:
    ...
    @dev Includes fee.
    @param i index of input token. Check pool.self.coins(i) to get coin address at ith index
...
def get_dx(i: uint256, j: uint256, dy: uint256) -> uint256:
    ...
    @param i index of input token. Check pool.self.coins(i) to get coin address at
           ith index
    @param j index of output token
...
def lp_price() -> uint256:
    ...
    return (
        3 * self.virtual_price * self.MATH.cbrt(price_oracle[0] * price_oracle[1])
    ) / 10**24
...
def calc_token_fee(
    ...
    @param amounts The amounts of self.coins being added to the pool.
...
def precisions() -> uint256[N_COINS]:  # <-------------- For by view contract.
    ...
    @return uint256[3] precisions of self.coins.
    """
